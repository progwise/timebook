...
const getClient = async () => {
  ...
  const endpoint = `${url}/api/graphql` ?? 'https://timebook.progwise.net'
...
export class StartTracking extends SingletonAction<TrackingSettings> {
  ...
  async onKeyDown(event: KeyDownEvent<TrackingSettings>): Promise<void> {
    ...
    try {
      ...
      await (response.currentTracking &&
        taskID === response.currentTracking?.task.id ? this.stopCurrentTracking(event.action) :
  ...
  async onPropertyInspectorDidAppear(
    event: PropertyInspectorDidAppearEvent<TrackingSettings>
  ): Promise<void> {
